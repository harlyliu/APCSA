  ...
  ///////////////////// constructors //////////////////////////////////
	...
	*/
	...
	public Picture rotate(double angle) { 
		angle = -angle;
		Pixel[][] pixels = this.getPixels2D();
		double largestRow = -100000;
		double smallestRow = 100000;
		double largestCol = -100000;
		double smallestCol = 100000;
		double newRow, newCol ;
		for (int row = 0; row < pixels.length; row++)
		{
		  for (int col = 0; col < pixels[0].length; col++)
		  {
			newRow = row*Math.cos(angle)-col*Math.sin(angle);
			newCol = row*Math.sin(angle) + col*Math.cos(angle);
			largestRow = Math.max(largestRow, newRow);
			smallestRow = Math.min(smallestRow, newRow);
			largestCol = Math.max(largestCol, newCol);
			smallestCol = Math.min(smallestCol, newCol);
		  }
		}
		Picture result = new Picture((int)(largestRow-smallestRow+1),(int)(largestCol-smallestCol+1));
		Pixel[][] resultPixels = result.getPixels2D(); 
		for (int row = 0; row < pixels.length; row++)
		{
		  for (int col = 0; col < pixels[0].length; col++)
		  {
			newRow = row*Math.cos(angle)-col*Math.sin(angle);
			newCol = row*Math.sin(angle) + col*Math.cos(angle);
			resultPixels[(int)(newRow-smallestRow)][(int)(newCol-smallestCol)].setColor(pixels[row][col].getColor());
		  }
		}
		for (int i = 1; i < resultPixels.length-1; i++){
			for (int j = 0; j < resultPixels[0].length; j++){
				if (resultPixels[i][j].getColor().equals(Color.WHITE)
					&&!resultPixels[i-1][j].getColor().equals(Color.WHITE)
					&&!resultPixels[i+1][j].getColor().equals(Color.WHITE)){
